...
fun createLoader(activity: FragmentActivity) : AlertDialog {
    ...
    loader.setIcon(R.mipmap.ic_launcher1)
...
fun uploadImageView(app: DepositDiaryApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Deposit Diary", "uploadTask.exception" + error)
        }
    }
...
fun updateAllDepositDiarys(app: DepositDiaryApp) {
    ...
    var depositRef = app.database.ref.child("deposits")
                                  .orderByChild("email")
    val userdonationRef = app.database.ref.child("user-deposits")
                                  .child(userId).orderByChild("uid")

    depositRef.equalTo(userEmail).addListenerForSingleValueEvent(
...
fun validatePhoto(app: DepositDiaryApp, activity: Activity) {
    ...
    {
        activity.navView.getHeaderView(0).imageView.setImageResource(R.mipmap.ic_launcher1)
        uploadImageView(app, activity.navView.getHeaderView(0).imageView)
    }
}
...
fun checkExistingPhoto(app: DepositDiaryApp, activity: Activity) {
    ...
    app.userImage = "".toUri()
    Log.v("Deposit Diary","checkExistingPhoto 1 app.userImage : ${app.userImage}")
    ...
    app.database.child("user-photos").orderByChild("uid")
        ...
        override fun onDataChange(snapshot: DataSnapshot ) {
            snapshot.children.forEach {
                ...
                Log.v("Deposit Diary","checkExistingPhoto 2 app.userImage : ${app.userImage}")
            }
            Log.v("Deposit Diary","validatePhoto 3 app.userImage : ${app.userImage}")
